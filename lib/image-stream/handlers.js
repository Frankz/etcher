 ...
 *   return readBufferFromImageDescriptor(fileDescriptor, 512, 0);
 * }).then((buffer) => {
 *   console.log(buffer);
 * });
 */
const readBufferFromImageDescriptor = (fileDescriptor, count, offset) => {
...
module.exports = {
  ...
  'application/gzip': (imagePath, options) => {
    return Bluebird.using(fs.openAsync(imagePath, 'r').disposer((fileDescriptor) => {
      ...
      // Determine the uncompressed size of the gzip file by reading
      // its `ISIZE`. The specification clarifies that this value is
      // just an estimation.
      return readBufferFromImageDescriptor(fileDescriptor, ISIZE_LENGTH, options.size - ISIZE_LENGTH).then((buffer) => {
  ...
  'application/x-xz': (imagePath, options) => {
    return Bluebird.using(fs.openAsync(imagePath, 'r').disposer((fileDescriptor) => {
      ...
      return lzma.parseFileIndexAsync({
        ...
        read: (count, offset, callback) => {
          readBufferFromImageDescriptor(fileDescriptor, count, offset).asCallback(callback);
        }
      });
    }).then((metadata) => {
      return {
